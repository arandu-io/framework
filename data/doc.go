...
// called database because that is what Illuminate calls it, and this package is
// now the old name pointing at them. It answers to two hesape packages:
//
...
//
// Tenant is one line -- g.Subject().Tenant -- and it used to live here, in the
// package that owns the SQL, which forced the cache and the filesystem to
// import the database in order to read a tenant. It is auth.Tenant now
// (docs/31:175). RULE 14 does not move with it: the tenant still comes from the
// Grant, never from a path, a body, a query or a header.
//
