...
func TestNoDocCommentPointsAtADeletedDirectory(t *testing.T) {
	// The whole module: the promise is about every symbol it exports, not
	// about one package.
	err := filepath.WalkDir(tests.ModuleRoot(t), func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return nil
		}
		if d.IsDir() {
